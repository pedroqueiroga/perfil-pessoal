...
class UniversidadeRegexes:
    def __init__(self, universidade):
        ...
        self.periodo = re.compile('(?:PER.ODO: *(\d).|(SEM ?PERIODIZA..O))')
        self.cadeira = re.compile(('({}) *?- *(.+)').format(self.formula_str))
        self.prereq = re.compile('PR.-REQUISITO:')
        self.prereq_None = re.compile(
            'N.o ?h. ?Pr.-Requisito ?para ?esse ?Componente ?Curricular.'
        )
        self.coreq = re.compile('CO-REQUISITO:')
        ...
        self.equiv = re.compile('EQUIVAL.NCIA:')
        self.ementa = re.compile('EMENTA:')
        self.carga_horaria = re.compile('\d{1,}')
        self.curso = re.compile('Curso: *(.*)')
        self.perfil = re.compile('Perfil: *(.*)')
        self.observacao_perfil = re.compile('OBSERVA..O PERFIL:')
